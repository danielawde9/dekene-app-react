...
import { CLOSING_ALLOWED, DEFAULT_EXCHANGE_RATE, TRANSACTION_TYPES } from "../utils/constant";
  ...
  persist(
    (set) => ({
      ...
      setTransactions: (transactionsOrUpdater) =>
        set((state) => {
          const transactions =
            typeof transactionsOrUpdater === "function"
              ? transactionsOrUpdater(state.transactions)
              : transactionsOrUpdater;
          return { transactions };
        }),
      ...
      handleDelete: (type, key) =>
        set((state) => {
          const updatedTransactions = { ...state.transactions };
          if (type === TRANSACTION_TYPES.PAYMENTS) {
            const itemToDelete = state.transactions[type].find((item) => item.key === key);
            if (itemToDelete && itemToDelete.deduction_source === "daniel") {
              const saleKey = itemToDelete.generatedSaleKey;
              updatedTransactions[TRANSACTION_TYPES.SALES] = updatedTransactions[TRANSACTION_TYPES.SALES].filter(
                (item) => item.key !== saleKey
              );
            }
          }

          updatedTransactions[type] = updatedTransactions[type].filter((item) => {
            if (item.key === key && item.is_autogenerated) {
      ...
      setUnpaidCredits: (unpaidCredits) => set({ unpaidCredits }),
      unpaidCredits: [],
    }),
    {
    ...
    try {
      ...
      if (difference_usd !== 0 || difference_lbp !== 0) {
        ...
        const creditTransaction = {
          ...
          paid_amount_usd: 0,
          paid_amount_lbp: 0,
          is_autogenerated: true, // Mark as auto-generated
        };
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (credit_payments && credit_payments.length > 0) {
        // Insert credit payments into 'credit_payments' table
        const creditPaymentsData = credit_payments.map(({ type, key, ...item }) => ({
          ...item,
          date,
          user_id: selectedUser,
          branch_id: selectedBranch,
        ...
        const { error: creditPaymentsError } = await supabase
          .from("credit_payments")
          .insert(creditPaymentsData);

        if (creditPaymentsError) throw creditPaymentsError;

        // Update credits with new paid amounts
        for (const payment of credit_payments) {
          ...
          const isPaidInUSD = newPaidAmountUSD >= creditData.amount_usd;
          const isPaidInLBP = newPaidAmountLBP >= creditData.amount_lbp;
          const status = isPaidInUSD && isPaidInLBP;
                ...
                >
                  <Form form={payCreditForm}>
                    ...
                    <Form.Item label="Total Amount USD">
                      <Input
                        value={formatNumber(currentCredit?.amount_usd)}
                        disabled
                      />
                    </Form.Item>
                    <Form.Item label="Total Amount LBP">
                      <Input
                        value={formatNumber(currentCredit?.amount_lbp)}
                        disabled
                      />
                    </Form.Item>
      ...
      render: (_, record) => (
        <>
          <Button
            ...
            disabled={record.is_autogenerated}
          >
            Edit
          </Button>
          <Popconfirm
            ...
            disabled={record.is_autogenerated}
          >
            <Button type="link" disabled={record.is_autogenerated}>
              Delete
            </Button>
          </Popconfirm>
        </>
    ...
    <Card title={title} style={{ marginTop: 20 }}>
      <Form
        ...
        initialValues={{
          ...
          paid_amount_usd: 0,
          paid_amount_lbp: 0,
          is_autogenerated: false, // Default to false
        }}
        onFinish={(values) => {
          const key = Date.now();
          if (
      ...
      >
        {renderFormFields()}
        <Form.Item name="is_autogenerated" initialValue={false} hidden>
          <Input />
        </Form.Item>
        <Form.Item>
...
const ConfirmationModal = ({
  ...
  return (
    <Modal
      ...
      width={800}
    >
      <p>Are you sure you want to close the day? {selectedDate.toLocaleString()}</p>
